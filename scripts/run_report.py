...
def main() -> None:
    ...
    if all_negative:
        md.append(
            ...
            "subperiod, matching the decay literature); (2) SSL pretraining does "
            "not help (S6): with the v2 corpus — construction-matched to the "
            "supervised pipeline — the pretrained primary still trails the "
            "from-scratch variant (n.s.); the remaining explanation is era "
            "non-stationarity (a 2008-2010 crisis-era corpus vs 2011-2021 "
            "evaluation), which gate (f)'s cross-era inversion independently "
            "corroborates; "
